...
class ClientController extends Controller
{
    ...
    public function detail(int $id): void
    {
        ...
        $archives = $this->db->fetchAll("
            ...
            WHERE r.agent_id = ?
            ORDER BY r.name ASC, bp.name ASC, ar.created_at DESC
        ", [$id]);
    ...
    public function catalogSearchAll(int $id): void
    {
        ...
        try {
            ...
            $countRow = $catalogDb->fetchOne(
                ($catalogDb instanceof \BBS\Core\ClickHouse
                    ? "SELECT uniqExact(path) as cnt FROM file_catalog WHERE agent_id = ? AND (file_name LIKE ? OR path LIKE ?)"
                    : "SELECT COUNT(DISTINCT path) as cnt FROM file_catalog WHERE agent_id = ? AND (file_name LIKE ? OR path LIKE ?)"),
                [$id, "%{$search}%", "%{$search}%"]
            );
            ...
            if (empty($pathRows)) {
                $this->json(['catalog_available' => true, 'files' => [], 'total' => $total, 'page' => $page, 'pages' => $pages]);
                return;
            }
